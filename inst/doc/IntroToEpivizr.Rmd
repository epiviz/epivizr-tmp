...
In this vignette we will look at colon cancer methylation data from the TCGA project and expression data from the gene expression barcode project. The `epivizrData` package contains methylation data from the Illumina 450kHumanMethylation beadarray processed with the `minfi` package. We useß expression data from the `antiProfilesData` bioconductor package.
