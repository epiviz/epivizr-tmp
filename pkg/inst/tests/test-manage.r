...
test_that("addDevice works for blocks", {
  ...
  mgr <- .startMGR(nonBlocking=nonBlocking)
...
#   mgr <- .startMGR()
#   tryCatch({
#     devId1 <- mgr$addDevice(gr1, "dev1", sendRequest=sendRequest); if (sendRequest) Sys.sleep(sleepTime);
#     devId2 <- mgr$addDevice(gr2, "dev2", sendRequest=sendRequest); if (sendRequest) Sys.sleep(sleepTime);
#     devId3 <- mgr$addDevice(gr3, "dev3", sendRequest=sendRequest, type="bp"); if (sendRequest) Sys.sleep(sleepTime);
#     
#     
